...
import com.ehaohai.robot.databinding.ActivityMapModeBinding;
import com.ehaohai.robot.model.Heart;
import com.ehaohai.robot.ui.cell.OnInputConfirmListener;
import com.ehaohai.robot.ui.viewmodel.MapModeViewModel;
...
public class MapModeActivity extends BaseLiveActivity<ActivityMapModeBinding, MapModeViewModel> {
    ...
    private void bind_() {
        ...
        CommonUtil.click(binding.mapStop, new Action() {
            @Override
            public void click() {
                boolean value = Boolean.TRUE.equals(obtainViewModel().start.getValue());
                if(!value){
                    Toast.makeText(MapModeActivity.this, "请先开启建图模式", Toast.LENGTH_SHORT).show();
                    return;
                }
    ...
    @Override
    protected ActivityMapModeBinding dataBinding() {
        return DataBindingUtil.setContentView(this, R.layout.activity_map_mode);
    }
    ...
    @Override
    public MapModeViewModel obtainViewModel() {
        return ViewModelProviders.of(this, ViewModelFactory.getInstance()).get(MapModeViewModel.class);
    }
